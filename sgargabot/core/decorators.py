    ...
    def _callable_n_times_within(f):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if len(last_n_calls) < times:
                ...
                raise TooManyCallsError(f'User {args[1].author.name} attempted to call the'
                                        f'command "{args[1].command}" more times than allowed (Command can '
                                        f'only be called {times} times). Command attempted: {args[1].message.content}.', time_remaining=time_remaining)
        return wrapper
    return _callable_n_times_within

def _get_last_n_calls(n, field_name: str, usercalled_objs: mongoengine.QuerySet):
    return getattr(usercalled_objs.fields(**{'slice__'+field_name: -1*n}).first(), field_name)
    ...
    def _callable_n_times_per_day(f):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if len(last_n_calls) < times:
                ...
                results.first().add_call(f.__qualname__)
                return await f(*args, **kwargs)
            elif date_to - last_n_calls[0].replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=config.TZ_ZONEINFO) > datetime.timedelta(hours=24):
                # Check if the n-th most recent call allows for another call
                # It's important that is strictly less than, since the current call should be included in the count
                results.update_one(**{'pop__' + field_name: -1})
                results.first().add_call(f.__qualname__)
                ...
                next_day = last_call.replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=config.TZ_ZONEINFO) + datetime.timedelta(days=1)
